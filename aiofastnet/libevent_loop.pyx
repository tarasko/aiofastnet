...
cdef extern from "libevent_backend.h":
    aiofn_loop_backend_t *aiofn_libevent_backend_new() noexcept nogil
    void aiofn_libevent_backend_free(aiofn_loop_backend_t *) noexcept nogil
...
def new_event_loop():
    ...
    cdef aiofn_loop_backend_t *backend = aiofn_libevent_backend_new()
    if backend == NULL:
