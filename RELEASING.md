...
1. Make a pull request "Prepare x.x.x release"
...
6. Commit the version and changelog changes, wait for its required checks to pass, merge pull request.
...
4. Creates the GitHub Release after PyPI publishing succeeds.
5. Uses the extracted changelog section verbatim as the GitHub Release description and attaches all distributions.
...
- Confirm that the GitHub Release contains the same notes as `CHANGES.md` and includes the source distribution and all wheels.
