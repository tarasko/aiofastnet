...
`call_soon()` receives a pointer to a frontend-owned `aiofn_loop_action_t` embedded directly in a `Handle`. The backend stores its native cancellation token in `action->backend_token`, owns the scheduling queue, and later calls `action->callback(action->callback_data)` on the loop thread. It must clear `backend_token` before invoking the callback and must never invoke it inline. Calls are delivered in FIFO order, but the backend is otherwise free to choose its native scheduling phase and queue implementation.

On successful scheduling, the backend borrows the action until callback invocation or cancellation; `SelectorLoopBase` keeps its containing handle alive in an intrusive pending list. Cancelling a backend-pending `Handle` calls `call_soon_cancel()`; cancelling a `TimerHandle` calls `call_at_cancel()` instead, giving the backend an opportunity to release its native scheduling resources immediately. User-visible cancellation remains a frontend property: a cancelled handle never executes user code. Either cancel operation synchronously removes its scheduled callback or timer and guarantees that the backend will never access the action again. It clears `backend_token` but does not invoke `callback`; the frontend then unlinks and releases the handle.
...
Proactor reads use frontend-owned buffers, so `read_stop()` and `recvfrom_stop()` have explicit stop completions. A stop prevents new buffer allocations but permits the single already-issued read to report one final completion. The stop operation completes only after that possible read callback and establishes that all borrowed frontend storage is quiescent. A backend that stops synchronously may invoke the stop callback inline; a backend with native cancellation in flight invokes it later from the event loop. The frontend must therefore establish its stopping state before calling the backend and allow the inline callback to reenter backend operations after quiescence. It may retain successful late data while user-visible reading is paused, but it must not deliver that data to the protocol until reading resumes. A resume requested while a stop is pending is recorded by the frontend; a new backend read is not started until the stop completion runs. Closing similarly waits for stop completion before unwrapping the native handle and releasing the transport or read buffer.
...
`call_at()` uses the same action representation as `call_soon()`, with an absolute deadline and its own cancellation operation, `call_at_cancel()`, distinct from `call_soon_cancel()`. An adapter for a library with asynchronous cancellation must detach the frontend action synchronously and may retain a separate native object until that library reports cancellation; it cannot retain or later complete the frontend action after the corresponding cancel operation succeeds.
